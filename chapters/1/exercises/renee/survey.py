...
class Table(object):
    ...
    def AddRecord(self, record):
        ...
        """
        self.records.append(record) # Won't they say "Records" is not defined here?
    ...
    def ExtendRecords(self, records):
        ...
        """
        self.records.extend(records) # Hm, using the keyword 'records' twice...? Once is the existing table, one is the chunk of new ones?
...
def main(name, data_dir='.'):
    resp = Respondents() # (A) New instance of the repondents class, basically a new table of the respondents
    resp.ReadRecords(data_dir) # (B) You read thru it and builds on python object (a record with attributes) per entry
    print 'Number of respondents', len(resp.records) # (C) Process and print the number of respondents

    preg = Pregnancies() # (D) Do all those same things for pregnancy records. "preg" is a new table of pregnancy records
    preg.ReadRecords(data_dir)
    print 'Number of pregnancies', len(preg.records)

# OK so that's what the main method does. But how do you use all the other stuff you prepared, like accessing info about each field? How do you actually use the table?
